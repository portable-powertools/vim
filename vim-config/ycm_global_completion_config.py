...
def FlagsForEnvironment(key):
    if flags_raw.has_key(key) and flags_raw.get( key ) != None:
        value = flags_raw[key]
        return [flag for flag in value.split(':') if len(flag) > 0]
    return []
...
flags_raw = {
    'IDE_INCLUDE_FLAGS' : os.environ.get('IDE_INCLUDE_FLAGS' )
...
def FlagsForFile(filename):
    ...
    if compilation_db_flags:
        ...
        final_flags = final_flags + FlagsForSystem()
        for key in flags_raw.keys():
            final_flags = final_flags + FlagsForEnvironment(key)
    return {
            'flags': final_flags,
            'do_cache': True
            }
